...
- 变更中心（ChangeBatch 驱动交付）

- AI 交付运行控制台

- 上下文中心（Memory / Knowledge / Context Pack）
...
super-dev-studio/
  ...
  docs/
    ...
    PRODUCT_REFACTOR_BLUEPRINT.md

    WORKSPACE_REFACTOR_RELEASE_NOTES.md
...
当前版本已完成一次以 `super-dev` 为驱动的工作台重构：



- 产品蓝图：`docs/PRODUCT_REFACTOR_BLUEPRINT.md`

- 发布说明：`docs/WORKSPACE_REFACTOR_RELEASE_NOTES.md`
...
- `SUPERDEV_STUDIO_API_RATE_LIMIT_ENABLED`????? API ???????? `true`

- `SUPERDEV_STUDIO_API_RATE_LIMIT_WINDOW`?????????? `1m`

- `SUPERDEV_STUDIO_API_RATE_LIMIT_MUTATION`???????????????? `24`

- `SUPERDEV_STUDIO_API_RATE_LIMIT_EXPENSIVE`???????????????? `10`

- `SUPERDEV_STUDIO_API_RATE_LIMIT_PIPELINE`?pipeline ??/???????????? `6`

???? `Windows + amd64 + Go 1.24+` ????`eino` ? Volcengine Agent Runtime ??????? Ark ????????????? `sonic/loader` ?????????????`VOLCENGINE_ARK_*` ?????? LLM ???????
...
?????`docs/QUALITY_EVIDENCE_PLAYBOOK.md`
...
- `变更中心`：创建/选择当前 `ChangeBatch`

- `交付运行`：启动运行（模拟/真实 super-dev）、查看进度、事件与交付结果

- `上下文中心`：统一查看 Context Pack、记忆库、知识库

- `项目设置`：维护项目默认技术栈与上下文策略
